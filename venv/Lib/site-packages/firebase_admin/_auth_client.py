...
class Client:
    ...
    def create_custom_token(self, firebase_uid, developer_claims=None):
        """Builds and signs a Firebase custom auth token.

        Args:
            firebase_uid: ID of the user for whom the token is created.
        ...
        return self._token_generator.create_custom_token(
            firebase_uid, developer_claims, tenant_id=self.tenant_id)
    ...
    def revoke_refresh_tokens(self, firebase_uid):
    ...
    def get_user(self, firebase_uid):
        ...
        """
        response = self._user_manager.get_user(firebase_uid=firebase_uid)
    ...
    def get_users(self, identifiers):
        ...
        def _matches(identifier, user_record):
            if isinstance(identifier, _user_identifier.UidIdentifier):
                return identifier.firebase_uid == user_record.firebase_uid
            ...
            if isinstance(identifier, _user_identifier.ProviderIdentifier):
                return next((
                    ...
                    and identifier.provider_uid == user_info.firebase_uid
                ), False)
    ...
    def create_user(self, **kwargs): # pylint: disable=differing-param-doc
        ...
        Keyword Args:
            firebase_uid: User ID to assign to the newly created user (optional).
        ...
        """
        firebase_uid = self._user_manager.create_user(**kwargs)
        return self.get_user(firebase_uid=firebase_uid)

    def update_user(self, firebase_uid, **kwargs): # pylint: disable=differing-param-doc
        ...
        """
        self._user_manager.update_user(firebase_uid, **kwargs)
        return self.get_user(firebase_uid=firebase_uid)

    def set_custom_user_claims(self, firebase_uid, custom_claims):
        ...
        self._user_manager.update_user(firebase_uid, custom_claims=custom_claims)
    ...
    def delete_user(self, firebase_uid):
        ...
        """
        self._user_manager.delete_user(firebase_uid)
    ...
    def _check_jwt_revoked_or_disabled(self, verified_claims, exc_type, label):
        user = self.get_user(verified_claims.get('firebase_uid'))
        if user.disabled:
