...
function initPinBox() {
  document.querySelector('.pin-prompt form').addEventListener(
    ...
    function (event) {
      ...
      fetch(`${document.location}?__debugger__=yes&cmd=pinauth&pin=${pin}&s=${encodedSecret}`)
        .then((res) => res.json())
        .then(({ auth, exhausted }) => {
          if (auth) {
            EVALEX_TRUSTED = true;
            ...
            alert(
              `Error: ${
                exhausted ? 'too many attempts.  Restart server to retry.' : 'incorrect pin'
              }`
            );
          }
        })
...
function promptForPin() {
  if (!EVALEX_TRUSTED) {
    const encodedSecret = encodeURIComponent(SECRET);
    fetch(`${document.location}?__debugger__=yes&cmd=printpin&s=${encodedSecret}`);
