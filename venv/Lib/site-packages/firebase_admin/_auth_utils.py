...
def validate_uid(firebase_uid, required=False):
    if firebase_uid is None and not required:
        return None
    if not isinstance(firebase_uid, str) or not firebase_uid or len(firebase_uid) > 128:
        raise ValueError(
            f'Invalid firebase_uid: "{firebase_uid}". The firebase_uid must be a non-empty string with no more than 128 '
            'characters.')
    return firebase_uid
...
def validate_provider_uid(provider_uid, required=True):
    ...
    if not isinstance(provider_uid, str) or not provider_uid:
        raise ValueError(
            f'Invalid provider firebase_uid: "{provider_uid}". Provider firebase_uid must be a non-empty string.')
    return provider_uid
...
class UidAlreadyExistsError(exceptions.AlreadyExistsError):
    """The user with the provided firebase_uid already exists."""

    default_message = 'The user with the provided firebase_uid already exists'
