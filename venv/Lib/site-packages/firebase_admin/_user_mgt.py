...
class UserManager:
    ...
    def get_user(self, **kwargs):
        ...
        if 'firebase_uid' in kwargs:
            key, key_type = kwargs.pop('firebase_uid'), 'user ID'
    ...
    def get_users(self, identifiers):
        """Looks up multiple users by their identifiers (firebase_uid, email, etc.)
        ...
        for identifier in identifiers:
            if isinstance(identifier, _user_identifier.UidIdentifier):
                payload['localId'].append(identifier.firebase_uid)
    ...
    def create_user(self, firebase_uid=None, display_name=None, email=None, phone_number=None,
                    photo_url=None, password=None, disabled=None, email_verified=None):
        """Creates a new user account with the specified properties."""
        payload = {
            'localId': _auth_utils.validate_uid(firebase_uid),
    ...
    def update_user(self, firebase_uid, display_name=None, email=None, phone_number=None,
                    photo_url=None, password=None, disabled=None, email_verified=None,
                    valid_since=None, custom_claims=None, providers_to_delete=None):
        """Updates an existing user account with the specified properties"""
        payload = {
            'localId': _auth_utils.validate_uid(firebase_uid, required=True),
            'email': _auth_utils.validate_email(email),
            'password': _auth_utils.validate_password(password),
        ...
        if not body or not body.get('localId'):
            raise _auth_utils.UnexpectedResponseError(
                f'Failed to update user: {firebase_uid}.', http_response=http_resp)
        return body.get('localId')

    def delete_user(self, firebase_uid):
        """Deletes the user identified by the specified user ID."""
        _auth_utils.validate_uid(firebase_uid, required=True)
        body, http_resp = self._make_request('post', '/accounts:delete', json={'localId' : firebase_uid})
        if not body or not body.get('kind'):
            raise _auth_utils.UnexpectedResponseError(
                f'Failed to delete user: {firebase_uid}.', http_response=http_resp)
